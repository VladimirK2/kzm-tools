...
[auth](#auth) | [compare](#compare) | [exec](#exec) | [hist](#hist) | [idesc](#idesc) | [list](#list) | [search](#search) | [tabl](#tabl) | [updrec](#updrec)
...
-var:{noempty}:1 - don't show empty fields.
...
Example when routine type is not known:

    trun tafcj - -s:tcjscripts\search.tcj -var:{tabl}:f.version -var:{tofind}:V.MB.CHILD.ACCOUNT.RULES

Output:

    # Table name: [F.VERSION]
    # Search string: [V.MB.CHILD.ACCOUNT.RULES]
    ACCOUNT,AO.PW
    ACCOUNT,AO.PW.JOINT
    ACCOUNT,CHILD.SB.LCY
    Records found: 3
...
Align IDs for better visibility:

    trun tafcj - -s:tcjscripts\tabl.tcj -var:{tabl}:f.printer.id -var:{idlen}:10
